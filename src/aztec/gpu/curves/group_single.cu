...
__device__ __forceinline__ void group_single<fq_single, fr_single>::load_affine(affine_element_single &X, affine_element_single &result) {
    fq_single::load(X.x, result.x);   
    fq_single::load(X.y, result.y);  
}

template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::load_jacobian(element_single &X, element_single &result) {
    fq_single::load(X.x, result.x);   
    fq_single::load(X.y, result.y);  
 ...
 */
template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::mixed_add(uint254 Y, uint254 Z, uint254 A, uint254 B, uint254 &res_x, uint254 &res_y, uint254 &res_z) {
...
template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::doubling(uint254 X, uint254 Y, uint254 Z, uint254 &res_x, uint254 &res_y, uint254 &res_z) {
    uint254 T0, T1, T2, T3;
 ...
 */
template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::add(uint254 X1, uint254 Y1, uint254 Z1, uint254 X2, uint254 Y2, uint254 Z2, uint254 &res_x, uint254 &res_y, uint254 &res_z) {
...
template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::load_projective(projective_element_single &X, projective_element_single &result) {
    fq_single::load(X.x.data[0], result.x.data[0]);      
    fq_single::load(X.x.data[1], result.x.data[1]);      
    fq_single::load(X.x.data[2], result.x.data[2]);      
    fq_single::load(X.x.data[3], result.x.data[3]);    
        
    fq_single::load(X.x.data[0], result.y.data[0]);      
    fq_single::load(X.x.data[1], result.y.data[1]);      
    fq_single::load(X.x.data[2], result.y.data[2]);      
    fq_single::load(X.x.data[3], result.y.data[3]);  

    fq_single::load(X.z.data[0], result.z.data[0]);      
    fq_single::load(X.z.data[1], result.z.data[1]);      
    fq_single::load(X.z.data[2], result.z.data[2]);      
    fq_single::load(X.z.data[3], result.z.data[3]);  
}

template <class fq_single, class fr_single> 
projective_element_single<fq_single, fr_single> group_single<fq_single, fr_single>::from_affine(const affine_element_single &other) {
 ...
 */
template <class fq_single, class fr_single> 
__device__ __forceinline__ void group_single<fq_single, fr_single>::add_projective(
uint254 X1, uint254 Y1, uint254 Z1, uint254 X2, uint254 Y2, uint254 Z2, uint254 &res_x, uint254 &res_y, uint254 &res_z) {
    ...
    fq_single::mul(3 * gpu_barretenberg::b, t02, X3);                  // X3 ← b3 * t02 
    ...
    fq_single::mul(3 * gpu_barretenberg::b, t04, t04);                 // t04 ← b3 * t04 
