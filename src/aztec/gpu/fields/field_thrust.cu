...
struct ZeroFunctor {
    __device__ bool operator()(const uint254 &x) {
        return (x.limbs[0] == 0) && (x.limbs[1] == 0) && (x.limbs[2] == 0) && (x.limbs[3] == 0); 
    }
};
...
struct EqualFunctor {
    __device__ bool operator()(const uint254 &x, const uint254 &y) {
        return (x.limbs[0] == y.limbs[0]) && (x.limbs[1] == y.limbs[1]) && (x.limbs[2] == y.limbs[2]) && (x.limbs[3] == y.limbs[3]); 
    }
};
...
struct AddFunctor {
    __device__ uint254 operator()(const uint254 &a, const uint254 &b) {
        uint254 res;
        const var *p;
        if (std::is_same_v<params, BN254_MOD_BASE>) {
            p = gpu_barretenberg_single::MOD_Q_BASE;
...
struct MulFunctor {
    __device__ uint254 operator()(const uint254 af, const uint254 bf) {
        const var *a = af.limbs;
        const var *b = bf.limbs;
        // We don't need T[5] (ie. "T[s+1]" in the paper) since with a 254-bit curve, overflow is impossible
        var T[5]{0, 0, 0, 0, 0};
...
struct SquareFunctor {
    __device__ uint254 operator()(const uint254 &x) {
        uint254 res = x;
        square_inplace(res);
        return res;
    }
