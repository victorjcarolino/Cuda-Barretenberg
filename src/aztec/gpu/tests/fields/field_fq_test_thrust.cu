#include "field_thrust.cu"
#include <assert.h> 

using namespace std;
using namespace std::chrono;
using namespace gpu_barretenberg_thrust;
...
__global__ void initialize_mont_mult(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0x2523b6fa3956f038, 0x158aa08ecdd9ec1d, 0xf48216a4c74738d4, 0x2514cc93d6f0a1bf });
        b.push_back({ 0xb68aee5e4c8fc17c, 0xc5193de7f401d5e8, 0xb8777d4dde671db3, 0xe513e75c087b0bb });
        expected.push_back({ 0x7ed4174114b521c4, 0x58f5bd1d4279fdc2, 0x6a73ac09ee843d41, 0x687a76ae9b3425c });
    }
}

__global__ void mont_mult(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &result) {
    thrust::transform(a.begin(), a.end(), b.begin(), result.begin(), MulFunctor());
}


/* -------------------------- Montgomery Multiplication Test -- Short Integers ---------------------------------------------- */

__global__ void initialize_mont_mult_short(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0xa, 0, 0, 0 });
        b.push_back({ 0xb, 0, 0, 0 });
        expected.push_back({ 0x65991a6dc2f3a183, 0xe3ba1f83394a2d08, 0x8401df65a169db3f, 0x1727099643607bba });
    }
}

/* -------------------------- Multiply - Square Consistency ---------------------------------------------- */

__global__ void initialize_mul_square_consistency(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0x329596aa978981e8, 0x8542e6e254c2a5d0, 0xc5b687d82eadb178, 0x2d242aaf48f56b8a });
        b.push_back({ 0x7d2e20e82f73d3e8, 0x8e50616a7a9d419d, 0xcdc833531508914b, 0xd510253a2ce62c });
    }
}


__global__ void mul_square_consistency(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &expected, thrust::device_vector<uint254> &result) {
    thrust::device_vector<uint254> t1;
    thrust::device_vector<uint254> t2;

    thrust::transform(a.begin(), a.end(), b.begin(), result.begin(), SubFunctor());
    t1 = result;

    thrust::transform(a.begin(), a.end(), b.begin(), result.begin(), AddFunctor());
    t2 = result;
    thrust::transform(t1.begin(), t1.end(), t2.begin(), expected.begin(), MulFunctor());

    thrust::transform(a.begin(), a.end(), result.begin(), SquareFunctor());
    t1 = result;

    thrust::transform(b.begin(), b.end(), result.begin(), SquareFunctor());
    t2 = result;

    thrust::transform(t1.begin(), t1.end(), t2.begin(), result.begin(), SubFunctor());
}

/* -------------------------- Multiply - Square Against Constants ---------------------------------------------- */

__global__ void initialize_sqr_check_against_constants(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0x329596aa978981e8, 0x8542e6e254c2a5d0, 0xc5b687d82eadb178, 0x2d242aaf48f56b8a });
        expected.push_back({ 0xbf4fb34e120b8b12, 0xf64d70efbf848328, 0xefbb6a533f2e7d89, 0x1de50f941425e4aa });
    }
}

__global__ void sqr_check_against_constants(thrust::device_vector<uint254> a, thrust::device_vector<uint254> &result) {
    thrust::transform(a.begin(), a.end(), result.begin(), SquareFunctor());
}

/* -------------------------- Add - Check Against Constants ---------------------------------------------- */

__global__ void initialize_add_check_against_constants(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0x7d2e20e82f73d3e8, 0x8e50616a7a9d419d, 0xcdc833531508914b, 0xd510253a2ce62c });
        b.push_back({ 0x2829438b071fd14e, 0xb03ef3f9ff9274e, 0x605b671f6dc7b209, 0x8701f9d971fbc9 });
        expected.push_back({ 0xa55764733693a536, 0x995450aa1a9668eb, 0x2e239a7282d04354, 0x15c121f139ee1f6 });
    }
}

__global__ void add_check_against_constants(thrust::device_vector<uint254> a, uint254 b, uint254 &result) {
    fq_single::add(a, b, result);
}

/* -------------------------- Subtract - Check Against Constants ---------------------------------------------- */

__global__ void initialize_sub_check_against_constants(thrust::device_vector<uint254> &a, thrust::device_vector<uint254> &b, thrust::device_vector<uint254> &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0xd68d01812313fb7c, 0x2965d7ae7c6070a5, 0x08ef9af6d6ba9a48, 0x0cb8fe2108914f53 });
        b.push_back({ 0x2cd2a2a37e9bf14a, 0xebc86ef589c530f6, 0x75124885b362b8fe, 0x1394324205c7a41d });
        expected.push_back({ 0xe5daeaf47cf50779, 0xd51ed34a5b0d0a3c, 0x4c2d9827a4d939a6, 0x29891a51e3fb4b5f });
    }
}
...
__global__ void initialize_to_montgomery_form(uint254 &a, uint254 &expected) {
    for (int i = 0; i < 4; i++) {
        a.push_back({ 0x01, 0x00, 0x00, 0x00 });
        expected.push_back({ 0xd35d438dc58f0d9d, 0xa78eb28f5c70b3d, 0x666ea36f7879462c, 0xe0a77c19a07df2f });
    }
